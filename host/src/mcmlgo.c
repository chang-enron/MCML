 ...
 *	initialized.
 ****/
REAL Rspecular(LayerStruct * Layerspecs_Ptr)
{
  REAL r1, r2;
 ...
 ****/
void LaunchPhoton(REAL Rspecular,
...
REAL SpinTheta(REAL g)
{
  ...
  if(g == 0.0) 
    ...
    REAL temp = (1-g*g)/(1-g+2*g*RandomNum());
...
{
#if DEBUG
  extern int maxir, minir, maxiz, miniz;
#endif
...
//    Roulette(Photon_Ptr);
  if (Photon_Ptr->w <= 0) Photon_Ptr->dead = 1;
}
