...
FILE *GetFile(char *Fname)
{
  ...
  do {
    if(firsttime && Fname[0]!='\0') { 
      ...
      printf("Run OpenCL : ./mcml o platform device items/group items/batch inputfile.mci\n");
      printf("Query OpenCL devices : ./mcml q \n");
      printf("(non-OpenCL mode) Input filename(or . to exit):");
      scanf("%s", Fname);
      firsttime = 0;
    }
...
void cl_ReadFnameFormat(FILE *File_Ptr, cl_InputStruct *In_Ptr)
{
  char buf[STRLEN];
...
void cl_ReadNumPhotons(FILE *File_Ptr, cl_InputStruct *In_Ptr)
{
  char buf[STRLEN];
...
void cl_ReadDzDr(FILE *File_Ptr, cl_InputStruct *In_Ptr)
{
  char buf[STRLEN];
...
void cl_ReadNzNrNa(FILE *File_Ptr, cl_InputStruct *In_Ptr)
{
  char buf[STRLEN];
...
void cl_ReadNumLayers(FILE *File_Ptr, cl_InputStruct *In_Ptr)
{
  char buf[STRLEN];
...
{
  char buf[STRLEN];//, msg[STRLEN];
  REAL d, n, mua, mus, g;	/* d is thickness. */
					...
					LayerStruct ** Layerspecs_PP)
{
  char msg[STRLEN];
  short i=0;
  REAL z = 0.0;	/* z coordinate of the current layer. */
...
void cl_ReadLayerSpecs(FILE *File_Ptr, 
					short Num_Layers,
					LayerStruct *Layerspecs_PP)
{
  char msg[STRLEN];
...
void cl_CriticalAngle( short Num_Layers, 
					LayerStruct * Layerspecs_PP)
{
  short i=0;
  REAL n1, n2;
...
//  char buf[STRLEN];
  ...
  cl_ReadFnameFormat(File_Ptr, In_Ptr);
  cl_ReadNumPhotons(File_Ptr, In_Ptr);
  cl_ReadDzDr(File_Ptr, In_Ptr);
  cl_ReadNzNrNa(File_Ptr, In_Ptr);
  cl_ReadNumLayers(File_Ptr, In_Ptr);

  cl_ReadLayerSpecs(File_Ptr, In_Ptr->num_layers, 
				&In_Ptr->layerspecs[0]);
  cl_CriticalAngle(In_Ptr->num_layers, &In_Ptr->layerspecs[0]);
}
/***********************************************************
...
void cl_InitOutputData(cl_InputStruct In_Parm, 
					OutStruct * Out_Ptr)
{
  short nz = In_Parm.nz;
  short nr = In_Parm.nr;
...
void cl_FreeData(cl_InputStruct In_Parm, OutStruct * Out_Ptr)
{
  short nz = In_Parm.nz;
  short nr = In_Parm.nr;
  short na = In_Parm.na;
...
void cl_Sum2DRd(cl_InputStruct In_Parm, OutStruct * Out_Ptr)
{
  short nr = In_Parm.nr;
  short na = In_Parm.na;
  short ir,ia;
...
short cl_IzToLayer(short Iz, cl_InputStruct In_Parm)
{
  short i=1;	/* index to layer. */
  short num_layers = In_Parm.num_layers;
  REAL dz = In_Parm.dz;
...
void cl_Sum2DA(cl_InputStruct In_Parm, OutStruct * Out_Ptr)
{
  short nz = In_Parm.nz;
  short nr = In_Parm.nr;
  short iz,ir;
  ...
  for(iz=0; iz<nz; iz++) {
    sum += Out_Ptr->A_z[iz];
    Out_Ptr->A_l[cl_IzToLayer(iz, In_Parm)] 
	  += Out_Ptr->A_z[iz];
  }
  Out_Ptr->A = sum;
}
...
void cl_Sum2DTt(cl_InputStruct In_Parm, OutStruct * Out_Ptr)
{
  short nr = In_Parm.nr;
  short na = In_Parm.na;
  short ir,ia;
...
void ScaleRdTt(InputStruct In_Parm, OutStruct *	Out_Ptr)
{
  ...
  scale2 = 1.0/(REAL)In_Parm.num_photons;
  Out_Ptr->Rd *= scale2;
  Out_Ptr->Tt *= scale2;
}

void cl_ScaleRdTt(cl_InputStruct In_Parm, OutStruct *	Out_Ptr)
{
  short nr = In_Parm.nr;
  short na = In_Parm.na;
  REAL dr = In_Parm.dr;
...
void ScaleA(InputStruct In_Parm, OutStruct * Out_Ptr)
{
  ...
  scale1 = 1.0/(REAL)In_Parm.num_photons;	
  for(il=0; il<=nl+1; il++)
...
void cl_ScaleA(cl_InputStruct In_Parm, OutStruct * Out_Ptr)
{
  short nz = In_Parm.nz;
  short nr = In_Parm.nr;
  REAL dz = In_Parm.dz;
...
void cl_SumScaleResult(cl_InputStruct In_Parm, 
					OutStruct * Out_Ptr)
{
  /* Get 1D & 0D results. */
  cl_Sum2DRd(In_Parm, Out_Ptr);
  cl_Sum2DA(In_Parm,  Out_Ptr);
  cl_Sum2DTt(In_Parm, Out_Ptr);
  
  cl_ScaleRdTt(In_Parm, Out_Ptr);
  cl_ScaleA(In_Parm, Out_Ptr);
}
...
void cl_WriteInParm(FILE *file, cl_InputStruct In_Parm)
{
  short i;
...
void cl_WriteResult(cl_InputStruct In_Parm, 
				 OutStruct Out_Parm,
				 char * TimeReport)
{
  FILE *file;
